...
impl Builder {
    ...
    pub fn impurity(mut self, impurity: Criteria) -> Self {
...
fn evaluate_feature(ctx: &SplitContext, f: usize) -> (f64, usize, f64) {
    ...
    for j in 0..col.len() - 1 {
        ...
        let ig = ctx.parent_impurity
            - (left_size  as f64 / ctx.n_samples as f64) * (ctx.impurity)(&left_counts,  left_size)
            - (right_size as f64 / ctx.n_samples as f64) * (ctx.impurity)(&right_counts, right_size);
...
impl DecisionTreeClassifier {
    ...
    pub fn fit(&mut self, features: ArrayView2<f64>, target: ArrayView1<usize>) {
        ...
        let impurity_fn: fn(&[usize], usize) -> f64 = match self.impurity {
    ...
    fn build_tree(
        ...
        let parent_impurity = impurity(&parent_counts, n_samples);
